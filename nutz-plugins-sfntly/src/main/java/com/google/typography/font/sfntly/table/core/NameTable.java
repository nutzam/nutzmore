...
public final class NameTable extends SubTableContainerTable implements Iterable<
	...
	/**
	 * Get the name as bytes for the specified name. If there is no entry for
	...
	/**
	 * Get the name entry record for the specified name. If there is no entry
	 * for the requested name then <code>null</code> is returned.
	 * 
	 * @param platformId
	 *            the platform id
	 * @param encodingId
	...
	private static class NameEntryId implements Comparable<NameEntryId> {
		...
		public int hashCode() {
			/*
			 * - this takes advantage of the sizes of the various entries and
			 * the fact that the ranges of their values have an almost zero
			 * probability of ever changing - this allows us to generate a
			 * unique hash at low cost - if the ranges do change then we will
			 * potentially generate non-unique hash values which is a common
			 * result
			 */
		 ...
		 *
		 * @return less than zero if this entry is less than the other; greater
		 *         than zero if this entry is greater than the other; and zero
		 *         if they are equal
		 *
	...
	/**
	 * An interface for a filter to use with the name entry iterator. This
	 * allows name entries to be iterated and only those acceptable to the
	...
	public static class Builder extends SubTableContainerTable.Builder<NameTable> {
		...
		/**
		 * Revert the name builders for the name table to the last version that
		 * came from data.
		 */
