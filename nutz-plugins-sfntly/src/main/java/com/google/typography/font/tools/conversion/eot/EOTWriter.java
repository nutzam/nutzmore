...
public class EOTWriter {
	...
	public WritableFontData convert(Font font) throws IOException {
		...
		index += writeUnicodeRanges(index, os2Table, writableFontData); // UnicodeRange{1,
																		// 2, 3,
																		// 4}
		index += writeCodePages(index, os2Table, writableFontData); // CodePageRange{1,
																	// 2}
		...
		index += writeReservedFields(index, writableFontData); // Reserved{1, 2,
																// 3, 4}
	...
	private long computeEotSize(int familyNameSize, int styleNameSize, int versionNameSize,
		...
		return 16 * ReadableFontData.DataSize.ULONG.size() +
				...
				(5 * ReadableFontData.DataSize.ULONG.size());
	}
