...
/**
 * Implement LZCOMP compression algorithm as defined in MicroType Express, part
 * of the EOT draft spec at {@link "http://www.w3.org/Submission/MTX/"}
 * 
...
public class LzcompCompress {
	...
	private void write(byte[] dataIn) {
		...
		// If we want to do runlengths, here's the place, but I'm not convinced
		// it's useful
	...
	// consider refactoring signature to return PotentialMatch object with
	// fields set...
		...
		if (maxIndexMinusIndex > 1) {
			...
			for (HashNode hNode = hashTable[pos]; hNode != null; prevNode = hNode, hNode = hNode.next) {
				...
				if (length > maxComputedLength) {
					int limit = length;
					if (limit > maxCostCacheLength)
						limit = maxCostCacheLength;
